...
output = []
with open('DCG_input.txt', 'r') as f:
    for i in range(100):
        text = f.readline()
        example = text
        for i in range(num_itterations):
            new_example = inference(example, coherence_classifier, bart_model, classifier_tokenizer, bart_tokenizer)
...
with open('DCG_output.txt', 'w') as f:
    for i in output:
        f.write(i + '\n')
