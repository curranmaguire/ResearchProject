...
class AdversarialVAE(nn.Module):
    ...
    def forward(self, sequences, seq_lengths, style_labels, content_bow, iteration, last_epoch):
        ...
        #for i in range(len(sequences)):
           # if len(sequences[i]) != seq_lengths[i]:
            #    print(f'incorrect seq length {len(sequences[i])}, {seq_lengths[i]}')

        embedded_seqs = self.dropout(self.embedding(sequences))
        packed_seqs = pack_padded_sequence(
            embedded_seqs, lengths=seq_lengths, batch_first=True, enforce_sorted=False)
    ...
    def update_average_style_emb(self, style_emb, style_labels):
        ...
        #style_emb = style_emb.cpu()
        neg_style_label = torch.LongTensor([0, 1]).cuda()
    ...
    def generate_sentences(self, input_sentences, latent_emb, inference=False):
        ...
        if not inference:
            ...
            sos_token_tensor = torch.LongTensor(
                [gconfig.predefined_word_index['<sos>']]).unsqueeze(0).repeat(input_sentences.size(0), 1).to(input_sentences.device)
            ...
            output_sentences = torch.zeros(
                mconfig.max_seq_len, input_sentences.size(0), mconfig.vocab_size, device=input_sentences.device)
            # initialize hidden state
            hidden_states = torch.zeros(
                input_sentences.size(0), mconfig.hidden_dim, device=input_sentences.device)
        ...
        # if inference mode is on
            ...
            sos_token_tensor = torch.LongTensor(
                [gconfig.predefined_word_index['<sos>']]).unsqueeze(0).to(input_sentences.device)
