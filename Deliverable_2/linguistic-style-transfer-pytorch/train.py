...
if __name__ == "__main__":
    ...
    for epoch in trange(mconfig.epochs, desc="Epoch"):
        ...
        for iteration, batch in enumerate(tqdm(train_dataloader, disable=True)):
    ...
    with open(gconfig.avg_style_emb_path, 'wb') as f:
        model.avg_style_emb[0] = model.avg_style_emb[0].cpu()
        model.avg_style_emb[1] = model.avg_style_emb[1].cpu()
        pickle.dump(model.avg_style_emb, f)
        model.avg_style_emb[0] = model.avg_style_emb[0].cuda()
        model.avg_style_emb[1] = model.avg_style_emb[1].cuda()
